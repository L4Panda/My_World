...
{ 
    ...
    void Start()
    {
        ...
        int numSpawns = (int)Rand(400,500); // Number of objects to spawn on the mesh

        var beach = Resources.FindObjectsOfTypeAll(typeof(GameObject)).Cast<GameObject>().Where(g=>g.tag=="Beach").ToList();
        var grass = Resources.FindObjectsOfTypeAll(typeof(GameObject)).Cast<GameObject>().Where(g => g.tag == "Grass").ToList();
        var mountain = Resources.FindObjectsOfTypeAll(typeof(GameObject)).Cast<GameObject>().Where(g => g.tag == "Mountain").ToList();
        ...
        { 
            ...
            if (randPos.y <= 12)
            {
                    ...
                    prefab = beach[(int)Rand(0, beach.Count)];
                
            }
            else if (randPos.y < 40 && randPos.y > 12)
            {              
                    prefab = grass[(int)Rand(0, grass.Count)];
               
            }
            else if (randPos.y >= 40)
            {  
                    prefab = mountain[(int)Rand(0, mountain.Count)];              
            }
